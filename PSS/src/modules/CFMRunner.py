...
class CFMRunner:
    ...
    def __init__(self, config: dict = None):
        ...
        dist_base = os.path.join(
            project_root, "third_party", "cicflowmeter", "runtime_bundle"
        )
        ...
        if not os.path.isdir(dist_base):
            setup_script = os.path.join(
                ...
                "third_party",
                "cicflowmeter",
                "build_cicflowmeter_with_runtime_bundle.sh",
            )
            raise FileNotFoundError(
                f"CICFlowMeter runtime bundle not found at {dist_base}. "
                f"Build it first with: bash {setup_script}"
            )

        dist_candidates = [
            d for d in os.listdir(dist_base) if d.startswith("CICFlowMeter")
        ]
        if not dist_candidates:
            raise FileNotFoundError(
                f"No CICFlowMeter distribution found in {dist_base}"
            )
        ...
        self.max_workers = (
            config["cfm"].get("max_workers", max(4, os.cpu_count() or 4))
            if config
            else max(4, os.cpu_count() or 4)
        )
        self.timeout_min = config["cfm"].get("timeout_min", 30) if config else 30

    def run_cfm_on_phased_pcaps(
        self, phase_base_dir: str, num_phases: int, store: bool = True
    ) -> Dict[int, str]:
        """
        ...
        for ph in range(1, num_phases + 1):
            ...
            if not os.path.exists(input_dir):
                ...
                continue
            tasks.append(
                (ph, input_dir, output_dir, True)
            )  # is_dir=True for full directory processing

        return self._execute_tasks(tasks, store)

    def run_cfm_on_single_basename(
        self, phase_base_dir: str, num_phases: int, basename: str, store: bool = True
    ) -> Dict[int, str]:
        """
        ...
        for ph in range(1, num_phases + 1):
            file_name = (
                f"p_{ph}_{basename}"
                if basename.endswith(".pcap")
                else f"p_{ph}_{basename}.pcap"
            )
            ...
            if not os.path.exists(input_file):
                ...
                continue
            tasks.append(
                (ph, input_file, output_dir, False)
            )  # is_dir=False for single file processing
    ...
    def _execute_tasks(self, tasks: List[tuple], store: bool) -> Dict[int, str]:
        ...
        print(
            f"Starting CICFlowMeter on {len(tasks)} tasks using {self.max_workers} workers"
        )
        results = {}
        if store:
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                futures = {
                    executor.submit(
                        self._process_single_phase, ph, input_path, output_dir, is_dir
                    ): (ph, output_dir)
    ...
    def _process_single_phase(
        self, phase_num: int, input_path: str, output_dir: str, is_dir: bool
    ) -> bool:
        """
        ...
        try:
            cmd = [
                ...
                "-cp",
                self.classpath,  # Key fix: lib/* loads ALL jars including jnetpcap
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, timeout=self.timeout_min * 60
            )
            ...
            if result.returncode == 0:
                ...
                print(
                    f"[Phase {phase_num}] Success, wrote {csv_count} CSV files "
                    f"to {output_dir}"
                )
                return True
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_phases", type=int, required=True, help="Number of phases"
    )
    parser.add_argument(
        "--basename",
        type=str,
    ...
    if args.run:
        ...
        if args.basename:
            runner.run_cfm_on_single_basename(
                args.dataset_dir, args.num_phases, args.basename, store=True
            )
        else:
            runner.run_cfm_on_phased_pcaps(
                args.dataset_dir, args.num_phases, store=True
            )
