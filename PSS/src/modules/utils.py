...
def canonical_flow_key(
    ...
    """Return a direction-independent key while keeping each IP paired with its port."""
    endpoint_a, endpoint_b = sorted(
        ((src_ip, int(src_port)), (dst_ip, int(dst_port)))
    )
    return (
        f"{endpoint_a[0]}-{endpoint_b[0]}-"
        f"{endpoint_a[1]}-{endpoint_b[1]}-{int(protocol)}"
    )
