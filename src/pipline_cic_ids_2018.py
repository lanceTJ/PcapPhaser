...
def main():
    ...
    for pcap_file in pcap_files:
        ...
        if not all_features_extracted:
            print(f"[PipeLine] Feature matrices not found for {pcap_basename}. Running FeatureExtractor and SingleFeatureMatrixBuilder.")
            ...
            print(f"[PipeLine] Feature matrices found for {pcap_basename}. Skipping extraction and matrix building.")
            matrices_data = {}
            for ft in feature_types:
        ...
        if not os.path.exists(merged_path):
            print(f"[PipeLine] Fusing features for {pcap_basename}.")
            fusion = FeatureFusion(config)
            # concat list of str to a str divided by comma
            fused_data = fusion.fuse_features(matrices_data, feature_types, merged_dir, pcap_basename, store=True)
        else:
            print(f"[PipeLine] Merged matrix found for {pcap_basename}. Loading.")
        ...
        for num_phases in num_phases_list:
            if num_phases < 2:
                print(f"[PipeLine] Skipping invalid num_phases: {num_phases}")
                continue
            ...
            if not os.path.exists(marks_path):
                print(f"[PipeLine] Dividing phases for {num_phases} phases on {pcap_basename}.")
                config['pss']['num_phases'] = num_phases  # Temporarily update for this run
                divider = PhaseDivider(config)
                phase_marks = divider.divide_phases(fused_data, marks_dir, pcap_basename, store=True)
            else:
                print(f"[PipeLine] Phase marks found for {num_phases} phases on {pcap_basename}. Loading.")
            ...
            if not all(os.path.exists(os.path.join(phased_pcap_root, f'phase_{ph}', f'p_{ph}_{phased_pcap_basename}')) for ph in range(1, num_phases + 1)):
                print(f"[PipeLine] Reconstructing phased pcaps for {num_phases} phases on {pcap_basename}.")
                ...
                print(f"[PipeLine] Phased pcaps found for {num_phases} phases on {pcap_basename}. Skipping reconstruction.")
            ...
            if not all(os.path.exists(os.path.join(cfm_output, f'phase_{ph}', f'p_{ph}_{phased_pcap_basename}_Flow.csv')) for ph in range(1, num_phases + 1)):
                print(f"[PipeLine] Running CFMRunner for {num_phases} phases on {pcap_basename}.")
                runner = CFMRunner(config)
                runner.run_cfm_on_phased_pcaps(phase_base_dir, num_phases, store=True)
            else:
                print(f"[PipeLine] CFM features found for {num_phases} phases on {pcap_basename}. Skipping CFM run.")
            ...
            if not os.path.exists(concat_path):
                ...
                print(f"[PipeLine] Concatenated features found for {num_phases} phases on {pcap_basename}. Skipping concatenation.")
            ...
            if not os.path.exists(labeled_path):
                ...
                print(f"[PipeLine] Labeled features found for {num_phases} phases on {pcap_basename}. Skipping labeling.")
