...
def process_pcap(pcap_file, args, config, feature_types, num_phases_list, feature_matrix_dir, dataset_dir, pcap_dir):
    """
...
def main():
    ...
    parser.add_argument('--input_dir', type=str, required=True, help='Path to input date directory.')
    parser.add_argument('--output_dir', type=str, required=True, help='Path to output date directory.')
    ...
    # Get pcap dir under input_dir, fallback to input_dir if 'pcap' subdir not found
    ...
    if os.path.exists(candidate_pcap_dir):
        ...
        print(f"[PipeLine] 'pcap' subdir not found, falling back to input_dir: {pcap_dir}")
