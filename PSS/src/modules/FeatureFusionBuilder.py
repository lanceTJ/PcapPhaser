...
@njit(fastmath=True)  # keep signature & compilation mode; no parallel to avoid List/parfor issues
def fuse_matrices(js_list: NumbaList[np.ndarray], weights: np.ndarray) -> np.ndarray:
    """
    Drop-in replacement for the original fuse_matrices(js_list, weights).

    Behavior:
    - Normalizes each J to [0,1] based on min/max computed over the *upper triangle incl. diagonal* only.
    - Accumulates only the upper triangle incl. diagonal into fused_j; lower triangle remains 0.
    - Assumes all Js have the same (n,n) shape.
    """
    ...
    n = js_list[0].shape[0]
    fused_j = np.zeros((n, n), dtype=np.float64)

    for i in range(num_mats):
        j_mat = js_list[i]
        wi = weights[i]

        # min/max over upper triangle incl diag
        mn = j_mat[0, 0]
        mx = j_mat[0, 0]
        for r in range(n):
            for c in range(r, n):
        ...
        if denom > EPS:
            inv = 1.0 / denom
            # accumulate normalized values over upper triangle incl diag
            for r in range(n):
                for c in range(r, n):
                    fused_j[r, c] += (j_mat[r, c] - mn) * inv * wi
        # else: add nothing (same intent as original: avoid division by ~0)
